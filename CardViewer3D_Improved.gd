...
@onready var card_front_renderer = $CardFrontRenderer/SubViewport
@onready var card_back_renderer = $CardBackRenderer/SubViewport
...
func _ready():
	...
	_configure_lighting()
	$TextureRect.gui_input.connect(_on_bg_gui_input)
...
func setup_card(bcard_data: Dictionary):
	...
	# Renderizar frente e verso da carta
	front_texture = await _render_card_to_texture(bcard_data, false)
	back_texture = await _render_card_to_texture(bcard_data, true)
...
func _render_card_to_texture(data: Dictionary, is_back: bool) -> Texture2D:
	var target_viewport = card_front_renderer if not is_back else card_back_renderer
	
	for child in target_viewport.get_children():
		child.queue_free()
	
	var card_2d = CardScene.instantiate()
	target_viewport.add_child(card_2d)
	card_2d.visible = true
	
	if is_back:
		card_2d.is_face_up = false
		card_2d.update_visibility()
	else:
		card_2d.is_face_up = true
		card_2d.update_visibility()
...
func _load_card_back_texture() -> Texture2D:
	# Esta função não será mais usada diretamente, pois o verso será renderizado pelo Card 2D
	# No entanto, mantemos para compatibilidade ou caso haja necessidade de um fallback específico
